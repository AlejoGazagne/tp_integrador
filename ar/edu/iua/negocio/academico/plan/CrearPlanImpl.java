...
public class CrearPlanImpl implements CrearPlan {
    ...
    public boolean crear(Plan plan) {
        ...
        // FALTA UNO (No se permite un año con el att plan != del plan que se recibe [ej. plan.getAnios().get(0).getPlan().equals(plan) == false, es error ])
        for(int ii = 0; ii < plan.getAnios().size(); ii++){
            ...
            // FALTA UNO (No se permite una materia con el att anioPlan != del anioPlan al q pertenece [ej. anioPlan.getMaterias().get(0).getAnioPlan().equals(anioPlan) == false, es error ])
